...
namespace IntroSE.Kanban.Backend.BussinessLayer
{
    public class BoardB
    {
        ...
        //tests
        public BoardB(string email,ColumnB[] cols)
        {
            this.email = email;
            columns = cols;
        }
        ...
        public virtual bool hasTask(int taskId)
        {
            for (int i = 0; i < columns.Length; i++)
            {
                if (columns[i].hasTask(taskId))
        ...
        {
            if (ColumnOrdinal>=0& ColumnOrdinal<=Columns.Length-1 &&columns[ColumnOrdinal].setLimit(lim))
            {
                return true;
            }
            return false;
        ...
        {
            if (column < columns.Length - 1 & column >= 0 && columns[column].UpdateTaskDueDate(id, dueDate))
            {
                return true;
            }
            return false;
        ...
        {
            if ((column < columns.Length - 1 & column >= 0) && columns[column].UpdateTaskTitle(id, title))
            {
                return true;
            }
            return false;
        ...
        {
            if ((column < columns.Length - 1 & column >= 0) && columns[column].UpdateTaskDescription(id, description) )
            {
                return true;
            }
            return false;
        ...
        public Column AddColumn(int ColumnOrdinal, string Name)
        {
            ...
            {
                if (col.Name == Name)
                    throw new KanbanException("column name already exists");
            }
